# python 3.7
import mitmproxy.http,json,os,m3u8,requests,shutil
...
class Counter:
    ...
    def __init__(self):
        self.Referer = 'https://xueyuan.xiaoe-tech.com/'
        self.Cookie = '请填写你的cookie'
    ...
    def response(self, flow: mitmproxy.http.HTTPFlow):
        ...
        # 导入全局变量
        global cryptos,filename,m3u8_data,result_filename,repair_file_py,title,finish_file_flag

        # 注入 JavaScript 
        # 启动就能点击播放器
        if 'detail' in flow.request.url:
        ...
        # 获取课程标题
        if 'xe.goods.detail.get' in flow.request.url:
            
            # 加载 JSON 对象
            json_data = json.loads(flow.response.text)

            # 获取当前视频标题
            title = json_data['data']['title'].replace(' ','')
        ...
        if '.m3u8' in flow.request.url:
            ...
            # 加载 m3u8 对象
            dict_data = m3u8.parse(flow.response.text)

            # 获取 m3u8 全部分片链接
            m3u8_data = [ data['uri'] for data in dict_data['segments']]
            print(m3u8_data)

            # 获取解密参数
            m3u8_content = requests.get(url=dict_data['keys'][0]['uri'],headers=self.headers).content
            cryptos = AES.new(m3u8_content,mode)

            # 将密钥 写入 修复文件
            repair_file_py = repair_file_py.format(str(m3u8_content))
            print('\n' + '-'*50)
        ...
        if '.ts' in flow.request.url:
            ...
            # 确定最后一个分片
            start_data = m3u8_data[-1].split('?')[1].split('&')[0]
            end_data = m3u8_data[-1].split('?')[1].split('&')[1]
            ...
            if cryptos != None:
                ...
                # 如果是最后一个分片，开始合成视频
                if result_data in m3u8_ts_filename:
                    
                    # 拷贝 ffmpeg.exe 写入指定目录
                    ...
                    # 读取 解密成功 标记文件
                    with open(m3u8_finish_file_filename,'r',encoding='utf-8') as f:
                        ...
                        if f.read()!='':
                            ...
                            print(os.path.exists(result_filename + '/' + filename.split('/')[-1] + '.mp4"'))
                            print(result_filename + '/' + filename.split('/')[-1] + '.mp4')
                            print('[异常]：当前视频只下载最后一片，将不会合成视频')
            else:
